...
def generate_gemini_response(prompt: str) -> str:
    ...
    if not llm:
        ...
        raise ValueError(
            "LLM not initialized. Check application startup and ingest logs."
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            ...
            info: CollectionInfo = qdrant_client.get_collection(
        ...
        if vector_size == 0:
            ...
            from app.ingest import (  # Import locally to avoid circular dependencies
                build_or_update_index,
            )

            app.state.index = build_or_update_index(
                app.state.config, app.state.vector_store, Settings.embed_model
            )

        app.state.health_score = _calculate_and_persist_health_score(
            rAGSystemInitializer.config,
            rAGSystemInitializer.vector_store,
...
async def delete_ingestion_data(request: Request):
    ...
    try:
        ...
        if "Not found" in str(e):
            qdrant_deleted = True
            print(
                f"INFO: Qdrant collection '{COLLECTION_NAME}' not found, treating as deleted."
            )
        else:
            print(
                f"ERROR: Failed to delete Qdrant collection (Unexpected Response): {e}"
            )
            raise HTTPException(
                status_code=500, detail=f"Failed to delete Qdrant collection: {e}"
            )
    except Exception as e:
    ...
    if os.path.exists(persistentPath):
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to delete local data directory: {e}"
            )
    else:
        local_data_deleted = True
        print(
            f"INFO: Local directory '{persistentPath}' not found, treating as deleted."
        )
...
async def rag_query(request: QueryRequest):
    ...
    try:
        ...
        if not retrieved_chunks:
            ...
            rag_prompt = f"""No relevant information was found in the knowledge base for the query: '{query}'.
            Please answer only with information you know generally about the topic, or state clearly that no specific context was found."""
            ...
            if RERANKER_MODEL and len(retrieved_chunks) > 0:
                print(
                    f"INFO: Starting fast cross-encoder reranking for {len(retrieved_chunks)} chunks."
                )
                ...
                print(
                    f"INFO: Reranking complete. Selected Top {len(final_context_chunks)} chunks for context."
                )
            else:
                final_context_chunks = (
                    retrieved_chunks  # Use all retrieved if no reranker/no chunks
                )
            ...
            rag_prompt = app.state.rag_prompt_template.format(
        ...
        return RAGResponse(answer=llm_answer, sources=sources_list)
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error during RAG process: {e}"
        )
...
if __name__ == "__main__":
    ...
    api_host = temp_config.get("API_HOST", "127.0.0.1")
