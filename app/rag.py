...
def get_embedding(text: str) -> List[float]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Embedding generation failed. Check Ollama server."
        )
...
def retrieve_chunks(query_vector: List[float]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError(
            f"Could could not perform search on Qdrant. Is the collection '{COLLECTION_NAME}' created?"
        )
    ...
    for i, hit in enumerate(search_result):
        ...
        if i == 0:
            print(
                f"DEBUG: Keys found in first retrieved payload: {list(hit.payload.keys())}"
            )
        ...
        try:
            chunk = {
                ...
                "text": hit.payload[
                    "_node_content"
                ],  # CORRECTED: Use LlamaIndex node content key
                "source": hit.payload[
                    "file_name"
                ],  # CORRECTED: Use a reliable file name key for attribution
            }
            retrieved_chunks.append(chunk)
        except KeyError as e:
            # Raise a specific error indicating payload structure issue
            print(
                f"FATAL: Missing key {e} in Qdrant payload! Actual keys: {list(hit.payload.keys())}"
            )
            raise KeyError(
                f"""Missing expected key '{e}' in Qdrant payload. Keys found: {list(hit.payload.keys())}.
                  Please check that your ingestion process is using '_node_content' for text and 'file_name' for source."""
            )
...
def generate_ollama_response(rag_prompt: str) -> str:
    ...
    """
    print(
        f"INFO: Calling Ollama LLM for generation with prompt length {len(rag_prompt)}"
    )
