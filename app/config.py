...
class RAGSystemInitializer:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"INFO: Configuration loaded successfully from {self.CONFIG_FILE_NAME}."
            )
            return config
    ...
    def initialize_system(self) -> QdrantVectorStore:
        ...
        Settings.llm = Gemini(
            ...
            temperature=self.config[
                "LLM_TEMPERATURE"
            ],  # Example: setting a low temperature for factual RAG
        )
        ...
        try:
            ...
            print(
                f"Qdrant connection successful. Collection: {self.config['QDRANT_COLLECTION_NAME']}"
            )
            ...
            print(
                f"FATAL: Could not connect to Qdrant at {self.config['QDRANT_HOST']}:{self.config['QDRANT_PORT']}. Is the service running?"
            )
            print(f"Error: {e}")
            sys.exit(1)
    ...
    def load_rag_prompt_template(self, filepath: str = "system_prompt.txt") -> str:
        ...
        try:
            ...
            print(
                f"FATAL ERROR: Prompt file '{filepath}' not found. Using hardcoded default."
            )
