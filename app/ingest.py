...
def _calculate_and_persist_health_score(
    config: Dict[str, Any], vector_store: QdrantVectorStore, embed_model: BaseEmbedding
):
    """
    ...
    SAMPLE_SIZE = config[
        "SAMPLE_SIZE"
    ]  # Number of vectors to sample from Qdrant for health check
    ...
    try:
        ...
        sampled_vectors = [
            np.array(p.vector) for p in points_batch if p.vector is not None
        ]

        if not sampled_vectors:
            print(
                "WARNING: Failed to retrieve sampled vectors. ICS calculation skipped."
            )
            return

        # 5. Calculate Average Cosine Similarity (ICS)
        similarities = [
            cosine_similarity(ref_vector_np, vec) for vec in sampled_vectors
        ]
        ...
        print(
            f"ICS calculated and persisted: {score_data['score']:.4f} (Sampled {len(sampled_vectors)} points)"
        )
...
def _build_full_index(
    storage_context: StorageContext, config: Dict[str, Any]
) -> BaseIndex:
    """
    ...
    print(
        f"Successfully loaded {len(documents)} source documents for full index build."
    )
    ...
    print(
        f"Full Indexing complete. Vectors stored in Qdrant. Total searchable nodes: {len(nodes)}"
    )
...
def build_or_update_index(
    ...
    if not os.path.exists(data_dir):
        ...
        sys.exit(
            "Please place source documents (PDFs, TXT) in the './data' directory and run again."
        )
    ...
    if os.path.exists(persist_dir):
        ...
        try:
            ...
            print(
                f"WARNING: Error loading index metadata: {e}. Falling back to full rebuild..."
            )
...
def ingest_documents():
    ...
    build_or_update_index(config, vector_store, Settings.embed_model)
